"""Debug route: single-call upload + process for rapid iteration."""

from __future__ import annotations

import asyncio
import uuid
import shutil
from datetime import datetime
from pathlib import Path

from fastapi import APIRouter, File, Form, UploadFile
from fastapi.responses import JSONResponse

from .. import config
from ..deps import get_db

router = APIRouter(prefix="/api/debug", tags=["debug"])

DEBUG_DIR = config.DATA_DIR / "debug"


@router.post("/process")
async def debug_process(file: UploadFile = File(...)):
    ext = Path(file.filename or "upload.jpg").suffix.lower()
    if ext not in config.ALLOWED_EXTENSIONS and ext not in {'.heic', '.HEIC'}:
        return JSONResponse({"error": f"不支持的文件类型: {ext}"}, status_code=400)

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    uid = uuid.uuid4().hex[:6]
    session_id = f"{ts}_{uid}"
    out_dir = DEBUG_DIR / session_id
    out_dir.mkdir(parents=True, exist_ok=True)

    tmp_path = out_dir / f"upload{ext}"
    contents = await file.read()
    tmp_path.write_bytes(contents)

    db = get_db()
    paper_id = db.create_paper(
        child_id="K1", subject="其他",
        original_path=str(tmp_path), title=f"Debug {session_id}"
    )
    if paper_id is None:
        paper_id = 0

    db.update_paper_status(paper_id, "processing")

    loop = asyncio.get_event_loop()
    try:
        from src.m1_image_engine.engine import process_paper
        result = await loop.run_in_executor(None, process_paper, str(tmp_path), str(out_dir))
    except Exception as e:
        db.update_paper_status(paper_id, "failed", error_message=str(e))
        return JSONResponse({"error": str(e)}, status_code=500)

    if not result.success:
        db.update_paper_status(paper_id, "failed", error_message=result.error or "unknown")
        return JSONResponse({
            "error": result.error or "处理失败",
            "warnings": result.warnings,
        }, status_code=500)

    db.update_paper_status(
        paper_id, "processed",
        processed_path=str(result.processed_path),
        cleaned_path=str(result.cleaned_path),
        quality_score=result.quality_score,
    )

    def _url(p):
        if p is None:
            return None
        rel = Path(p).relative_to(Path.cwd()) if Path(p).is_relative_to(Path.cwd()) else Path(p)
        return f"/static/data/{rel.as_posix()}"

    return {
        "session_id": session_id,
        "quality_score": result.quality_score,
        "warnings": result.warnings,
        "stages": {
            "original": _url(result.original_path),
            "processed": _url(result.processed_path),
            "red_mask": _url(result.red_mask_path),
            "handwriting_mask": _url(result.hw_mask_path),
            "combined_mask": _url(result.combined_mask_path),
            "cleaned": _url(result.cleaned_path),
        },
    }


@router.post("/cleanup")
async def debug_cleanup():
    try:
        shutil.rmtree(DEBUG_DIR, ignore_errors=True)
        return {"status": "cleaned"}
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)
